...
def description_and_title_and_category_lookup(entities, content, content_len,
                                 title, title_len, categories, categories_len, vocab_table, word_embedding, str_pad, name=None):
    """ A convenience function for looking up both content and title embeddings and category embeddings.
    ...
    """
    varlist = [entities, content, content_len,
               title, title_len, categories, categories_len, vocab_table, word_embedding]
    with tf.name_scope(name, 'desc_title_lookup', varlist):
        flatten_entities = tf.reshape(entities, [-1], 'flatten_entities')
        ...
        categories_embedding, categories_true_len = entity_content_embedding_lookup(flatten_entities,
                                                                          categories,
                                                                          categories_len,
                                                                          vocab_table,
                                                                          word_embedding,
        ...
        # reshape [-1, content length, word_embedding_size]
        # to [????, content length, word_embedding_size]
        content_embedding, title_embedding, categories_embedding = [tf.reshape(x,
                                                         tf.concat([tf.shape(entities),
                                                                    tf.shape(x)[1:]], axis=0),
                                                         y) for x, y in
                                              zip([content_embedding, title_embedding, categories_embedding],
                                                  ['content_word_embedding',
                                                   'title_word_embedding',
                                                   'categories_word_embedding'])]

        tf.logging.debug("content_embedding %s title_embedding %s" % (content_embedding,
                                                                      title_embedding))

        content_true_len, title_true_len, categories_true_len = [tf.reshape(x, tf.shape(entities), y) for x, y in
                                            zip([content_true_len, title_true_len,categories_true_len],
                                                ['content_sequence_len',
                                                 'title_sequence_len',
                                                 'categories_sequence_len'])]

        return (content_embedding, content_true_len), (title_embedding, title_true_len), (categories_embedding, categories_true_len)
