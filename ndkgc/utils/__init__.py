...
def load_categories(category_file_path,entities):
    cat = dict()
    d = dict()
    l = dict()
    ...
    for key, values in cat.items(): 
        if key in entities:
            d[entities[key]]=' '.join(values)
    categories = list()
    categories_len = list()
    for i in range(len(d)):
        categories.append(d[i])
        categories_len.append(len(d[i]))
    tf.logging.info("Load %d category data from %s" % (len(d), category_file_path))
    return categories,categories_len
