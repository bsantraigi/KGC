...
from ndkgc.models.content_model_v2 import ContentModel
...
### MAKE CHANGES HERE
        self.predict_weight = tf.Variable([[[[1.]]] * 10] * self.n_relation, trainable=True, name='predict_weight')
    ...
    def translate_triple(self, heads, tails, rels, device, reuse=True):
        with tf.name_scope("fcn_translate_triple"):
            ...
            # right now
### MAKE CHANGES TO GET CATEGORY EMBEDDINGS OF HEAD AND TAIL
            ...
            transformed_rels = self._transform_relation(rels,
                                                        reuse=reuse,
                                                        device=device)
### Both functions should return categories
            transformed_head_avg, transformed_head_content, transformed_head_title, head_categories  = self._transform_head_entity(heads,
                                                                                                                 transformed_rels,
                                                                                                                 reuse=reuse,
                                                                                                                 device=device)
            transformed_tail_avg, transformed_tail_content, transformed_tail_title, tail_categories = self._transform_tail_entity(tails,
                                                                                                                 transformed_rels,
                                                                                                                 reuse=True,
                                                                                                                 device=device)
...
### NEED TO PASS CATEGORIES OF HEAD AND TAIL
    ...
    def _predict(self, head_content, head_content_avg, head_title, head_categories, rel_title, tail_content, tail_content_avg,
                 tail_title, tail_categories, rel_ids, device='/cpu:0', reuse=True,
                 name=None):
### NEED TO RECEIVE HEAD AND TAIL CATEGORIES, MATMUL TO GET SIMILARITY SCORE, GET MAX SCORE, MULTIPLY WITH WEIGHT
        with tf.name_scope(name, 'predict',
                           [head_content, head_content_avg, head_title, head_categories,
                            rel_title, tail_content, tail_content_avg, tail_title, tail_categories, rel_ids,
                            self.predict_weight]):
            with tf.variable_scope(self.pred_scope, reuse=reuse):
                with tf.device(device):
                    head_content, head_content_avg, head_title, head_categories, \
                    rel_title, \
                    tail_content, tail_content_avg, tail_title, tail_categories = [normalized_embedding(x) for x in
                                                                  [head_content, head_content_avg, head_title, head_categories,
                                                                   rel_title, tail_content, tail_content_avg,
                                                                   tail_title, tail_categories]]
                    ...
                    tail_categories = tf.cond(tf.equal(tf.rank(tail_categories), 3), lambda: tail_categories,
                                         lambda: tf.expand_dims(tail_categories, axis=1))
                    head_categories = tf.cond(tf.equal(tf.rank(head_categories), 3), lambda: head_categories,
                                         lambda: tf.expand_dims(head_categories, axis=1))
                    assert_ops = [tf.Assert(tf.equal(tf.rank(head_content), 3), [head_content]),
                                  ...
                                  tf.Assert(tf.equal(tf.rank(head_categories), 3), [head_categories]),
                                  tf.Assert(tf.equal(tf.rank(tail_categories), 3), [tail_categories]),
                    ...
                    with tf.control_dependencies(assert_ops):
                        ...
                        cat_sim = predict_helper(head_categories,tail_categories)
                        ...
                        semantic_sim2 = predict_helper(head_content_avg, tiled_tail_title)
                        semantic_sim3 = predict_helper(tail_content_avg, tiled_head_title)
    ...
    def __transform_entity(self, ents, transformed_rels, reuse=True, device='/cpu:0', name=None):
        ...
        :return:
        """
### ADD CALL TO CATEGORY LOOKUP, AND GET CATEGORY TENSOR FOR BOTH HEAD AND TAIL, AVG EACH CATEGORY
        varlist = [ents, transformed_rels, self.word_embedding, self.vocab_table, self.entity_content,
                   ...
                   self.is_train, self.categories, self.categories_len]

        with tf.name_scope(name, 'transform_entity', varlist):
            (ent_content, ent_content_len), (ent_title, ent_title_len), (ent_categories, ent_categories_len) = description_and_title_and_category_lookup(ents,
                                                                                                      self.entity_content,
            ...
            with tf.device(device):
                ...
                # Category averaging
                ent_categories = avg_content(ent_content, ent_content_len,pad_word_embedding,name = 'avg_category')
                ...
                return avg_entity, extracted_ent_content, avg_title, ent_categories
    ...
    def manual_eval_head_ops_v2(self, device='/cpu:0'):
        ...
        with tf.name_scope("manual_evaluation_v2"):
            with tf.device(device):
                ...
                pre_computed_head_queue = tf.FIFOQueue(1000000, dtypes=[tf.float32, tf.float32, tf.float32, tf.float32],
                                                       shapes=[[self.word_embedding_size]] * 4,
                                                       name='head_queue')
                ...
                computed_content_avg_heads, computed_content_heads, computed_title_heads, computed_head_categories = [tf.squeeze(x, axis=0) for x
                                                                                            in
                ...
                pre_compute_heads = pre_computed_head_queue.enqueue_many([
                    computed_content_avg_heads, computed_content_heads, computed_title_heads, computed_head_categories
                ])

                # get pre-computed heads from target queue
                dequeue_op = pre_computed_head_queue.dequeue_many(ph_target_size)
                head_coontent_avg, head_content_embeds, head_title_embeds, head_categories = [tf.expand_dims(x, axis=0) for x in
                                                                             dequeue_op]

                with tf.control_dependencies(dequeue_op):
                    re_enqueue = pre_computed_head_queue.enqueue_many(dequeue_op)

                computed_content_avg_tails, computed_content_tails, computed_title_tails, computed_tail_categories = self._transform_tail_entity(
    ...
    def manual_eval_tail_ops_v2(self, device='/cpu:0'):
        ...
        with tf.name_scope("manual_evaluation_v2"):
            with tf.device(device):
                ...
                pre_computed_tail_queue = tf.FIFOQueue(1000000, dtypes=[tf.float32, tf.float32, tf.float32, tf.float32],
                ...
                computed_rels = self._transform_relation(rels, reuse=True, device=device)
                computed_content_avg_tails, computed_content_tails, computed_title_tails,computed_tail_categories = [tf.squeeze(x, axis=0) for x
                                                                                            in
                ...
                pre_compute_tails = pre_computed_tail_queue.enqueue_many(
                    [computed_content_avg_tails, computed_content_tails, computed_title_tails, computed_tail_categories])

                # get pre-computed tails from target queue
                dequeue_op = pre_computed_tail_queue.dequeue_many(ph_target_size)
                tail_content_avg, tail_content_embeds, tail_title_embeds, tail_categories = [tf.expand_dims(x, axis=0) for x in
                                                                            dequeue_op]
                ...
                computed_content_avg_heads, computed_content_heads, computd_title_heads, computed_head_categories = self._transform_head_entity(
                    heads, computed_rels,
...
def main(_):
    ...
    model = FCNModel(entity_file=os.path.join(dataset_dir, 'entities.txt'),
                     ...
                     category_file = os.path.join(dataset_dir,'cat_dict.pickle'),
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            if os.path.exists(os.path.join(CHECKPOINT_DIR, '_checkpoint')):
        ...
        if is_train:
            ...
            try:
                ...
                while not coord.should_stop() and global_step <= 50100:
                    ...
                    if is_train:
                        ...
                        print("global_step %d loss %.4f" % (global_step, loss))
